...
displayStatus_t displayModeWrite( displayMode_t mode )
{
	...
	if ( mode == DISPLAY_MODE_CHAR ) {
		displayCommandWrite(0x30);  // 8 bit mode
		delay (1);
	} else if ( mode == DISPLAY_MODE_GRAPHIC ) {
		displayCommandWrite(0x30);  // 8 bit mode
		delay (1);
                                           ...
                                           bool pixelValue )
{
    if ( pixelPositionY < numRows && pixelPositionX < numCols ) {
		...
		*image = *p;
		...
		if ( startRow > pixelPositionY ) { 
			startRow = pixelPositionY;
		}
		if ( endRow <= pixelPositionY ) {
			endRow = pixelPositionY + 1;
		}
		if ( startCol > pixelPositionX ) {
			startCol = pixelPositionX;
		}
		if ( endCol <= pixelPositionX ) {
			endCol = pixelPositionX + 1;
		}
    }
