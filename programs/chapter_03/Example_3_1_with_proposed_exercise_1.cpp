...
void bleTask()
{
    if ( alarmLastTransmittedState != alarmState ) {
        if ( alarmState ) {
            uartBle.printf("ALARM_ON\r\n");
        } else {
            uartBle.printf("ALARM_OFF\r\n");
        }
        alarmLastTransmittedState = alarmState;
    }

    if ( gasLastTransmittedState != gasDetector ) {
        if ( gasDetector ) {
            uartBle.printf("GAS_DET_ON\r\n");
        } else {
            uartBle.printf("GAS_DET_OFF\r\n");
        }
        gasLastTransmittedState = gasDetector;
    }

    if ( tempLastTransmittedState != overTempDetector ) {
        if ( overTempDetector ) {
            uartBle.printf("OVER_TEMP_ON\r\n");
        } else {
            uartBle.printf("OVER_TEMP_OFF\r\n");
        }
        tempLastTransmittedState = overTempDetector;
    }
    
    if ( ICLastTransmittedState != incorrectCodeLed ) {
        if ( incorrectCodeLed ) {
            uartBle.printf("LED_IC_ON\r\n");
        } else {
            uartBle.printf("LED_IC_OFF\r\n");
        }
        ICLastTransmittedState = incorrectCodeLed;
    }

    if ( SBLastTransmittedState != systemBlockedLed ) {
        if ( systemBlockedLed ) {
            uartBle.printf("LED_SB_ON\r\n");
        } else {
            uartBle.printf("LED_SB_OFF\r\n");
        }
        SBLastTransmittedState = systemBlockedLed;
    }

    if ( aButtonLastTransmittedState != aButton ) {
        if( aButton ) {
            uartBle.printf("A_PRESSED\r\n");
        } else{
            uartBle.printf("A_RELEASED\r\n");
        }
        aButtonLastTransmittedState = aButton;
    }

    if ( bButtonLastTransmittedState != bButton ) {
        if( bButton ) {
            uartBle.printf("B_PRESSED\r\n");
        } else{
            uartBle.printf("B_RELEASED\r\n");
        }
        bButtonLastTransmittedState = bButton;
    }

    if ( cButtonLastTransmittedState != cButton ) {
        if( cButton ) {
            uartBle.printf("C_PRESSED\r\n");
        } else{
            uartBle.printf("C_RELEASED\r\n");
        }
        cButtonLastTransmittedState = cButton;
    }

    if ( dButtonLastTransmittedState != dButton ) {
        if( dButton ) {
            uartBle.printf("D_PRESSED\r\n");
        } else{
            uartBle.printf("D_RELEASED\r\n");
        }
